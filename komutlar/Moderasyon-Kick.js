...
exports.run = (client, message, args) => {
   ...
   var kisi = message.mentions.members.first() || client.users.resolve(args[0]) || client.users.cache.find(u => u.username === args[0]) || client.users.cache.get(args[0]); 
     ...
     if (!message.member.hasPermission("BAN_MEMBERS")) {
        const yetkinyok = new discord.MessageEmbed()
          .setDescription("**❌ Herhangi Bir Kişi Banlamak İçin `Kickle` İznine Sahip Olmalısın!**")
          .setColor("RED")
    ...
    .setDescription("**❌ Belirttiğiniz Kişi Sunucuda Yok Veya Kicklemek İçin Herhangi Bir Kişi Belirtmedin!**")
    .setColor("RED")
  ...
  if(message.member.roles.highest.comparePositionTo(message.mentions.members.first().roles.highest) <= 1) {
    ...
    .setDescription("**❌ Bu Kişi Rol Sıralamasında Senden Yüksekte Veya Eşit Bu Sebeple Onu Kicklemesin!**")
    .setColor("RED")
   ...
   .setDescription("**❌ Bu Kişi Sunucu Sahibi Onu Kicklemesin!**")
   .setColor("RED")
   return message.channel.send(arkadaşownermış) 
       ...
       .setDescription("**❌ Herhangi Bir Kişi Banlamak İçin `Kickle` İznine Sahip Olmalıyım!**")
       .setColor("RED")
 ...
 .setDescription("**❌ Bu Kişiyi Kicklemem!**")
 .setColor("RED")
 ...
 if(sebeb) {
   sebepp = sebeb
         ...
         try {
           ...
           .setDescription(`${kisi} **${guild}** Adlı Sunucudan Kickledim.` + "\r\n" + `**Sebep: ${sebepp}**`)
           .setColor("RED")
          kisi.send(sucembeddm)
           const sucembed = new discord.MessageEmbed()
           .setDescription(`✅ ${kisi} **Adlı Kullanıcı ${sebepp} Sebebi İle Kicklendi.**`)
           .setColor("GREEN")
...
exports.help = {
  name: "kick",
  description: "Belirttiğiniz Kişiyi Sunucudan kickler",
  usage: "kick <Etiket> <Sebep Varsa Sebep>"
};