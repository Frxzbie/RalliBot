...
message.guild.channels.create('🎁・etkinlikler', {type: 'text'}).then(c => c.setParent(parent.id));
message.guild.channels.create('👑・yönetim・ekibi', {type: 'text'}).then(c => c.setParen(parent.id));
message.guild.channels.create('🎓・davet', {type: 'text'}).then(c => c.setParen(parent.id));
message.guild.channels.create('⏯・otorol', {type: 'text'}).then(c => c.setParen(parent.id));
message.guild.channels.create('🚪・gelen-giden', {type: 'text'}).then(c => c.setParen(parent.id));

});


message.guild.channels.create('[sunucu ismi] Updates', {type: 'category'}).then(parent => {
message.guild.channels.create('📌・güncellemeler', {type: 'text'}).then(c => c.setParent(parent.id));
message.guild.channels.create('📌・haberler', {type: 'text'}).then(c => c.setParent(parent.id));
message.guild.channels.create('🏆・aylık・top・3', {type: 'text'}).then(c => c.setParent(parent.id));
});
  
message.guild.channels.create('topluluk kanalları', {type: 'category'}).then(parent => {
message.guild.channels.create('💬・sohbet', {type: 'text'}).then(c => c.setParent(parent.id));
message.guild.channels.create('🎀・resim', {type: 'text'}).then(c => c.setParent(parent.id));
message.guild.channels.create('⏯・komut', {type: 'text'}).then(c => c.setParent(parent.id));
});

message.guild.channels.create('sunucu marketi', {type: 'category'}).then(parent => {
message.guild.channels.create('💰・yükleme・yapanlar', {type: 'text'}).then(c => c.setParent(parent.id));
message.guild.channels.create('🎀・market・alışverişi', {type: 'text'}).then(c => c.setParent(parent.id));
})
});
